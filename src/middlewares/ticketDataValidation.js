...
const validateName = body('name')
	...
	.isLength({ min: 3 })
	.withMessage(
		"Name is a mandatory field, it's shoul contain min 3 characters."
	);
...
const validateDescription = body('description')
	...
	.isLength({ min: 10 })
	.withMessage(
		"Description is a mandatory field, it's shoul contain min 10 characters."
	);
...
const validateDate = body('date')
	...
	.isDate()
	.withMessage(
		'Invalid date, should be in yyyy-mm-dd format and in the future'
	)
	.isAfter()
...
const validateQuantity = body('quantity')
	.not()
	.isEmpty()
	.withMessage('Quantity is a mandatory field.')
	.isNumeric()
	.withMessage('Quantity must be numeric.')
	.isInt({ gt: 0 })
	.withMessage('Quantity value should be positive integer number.');

const validatecanCancel = body('canCancel')
	.isBoolean()
	.withMessage('Cancelability must be boolean.')
	.default(false);

const validateCancelDate = body('cancelDate')
	.isDate()
	.withMessage(
		'Invalid date, should be in yyyy-mm-dd format and in the future'
	)
...
const isBefore = (req, res, next) => {
	if (req.body.date < req.body.cancelDate) {
		res.status(400).json({
			error: 'Cancel date must be before ticket date.',
		});
